<x-admin-layout>
    <x-admin.page-header title="Edit Bank Account" icon="edit" backRoute="{{ route('admin.bank-accounts.index') }}" />
    <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 max-w-3xl">
        <form action="{{ route('admin.bank-accounts.update', $bankAccount) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <x-text-input id="bank_name" name="bank_name" type="text" value="{{ old('bank_name', $bankAccount->bank_name) }}" required />
            </div>
            <div class="mb-6">
                <x-input-label for="account_number" value="Account Number" required />
                <x-text-input id="account_number" name="account_number" type="text" value="{{ old('account_number', $bankAccount->account_number) }}" required />
            </div>
            <div class="mb-6">
                <x-input-label for="account_name" value="Account Name" required />
                <x-text-input id="account_name" name="account_name" type="text" value="{{ old('account_name', $bankAccount->account_name) }}" required />
            </div>
            <div class="mb-6">
                ...
                <x-select-input name="subsidiary_id" id="subsidiary_id" required>
                    @foreach($subsidiaries as $sub)
                        <option value="{{ $sub->id }}" {{ (old('subsidiary_id', $bankAccount->subsidiary_id) == $sub->id) ? 'selected' : '' }}>{{ $sub->name }}</option>
                    @endforeach
                </x-select-input>
            </div>
            <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-outline-variant/20">
                ...
                <x-primary-button>
                    <x-lucide-save class="w-4 h-4 mr-2" /> Update Account
                </x-primary-button>
            </div>
        </form>
    </div>
