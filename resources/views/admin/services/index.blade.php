<x-admin-layout>
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-on-surface">Kelola Layanan</h1>
                <p class="text-lg text-on-surface-variant mt-1">Kelola jenis-jenis layanan yang ditawarkan oleh perusahaan.</p>
            </div>
        ...
        <x-admin.table :headers="['Ikon', 'Judul Layanan', 'Deskripsi Singkat', 'Status', 'Aksi']">
            @forelse($services as $service)
                <tr>
                    ...
                    <td class="px-6 py-4 font-semibold text-on-surface text-lg">
                        {{ $service->title }}
                        <span class="block text-sm text-outline font-normal mt-0.5">EN: {{ $service->title_en ?? '-' }}</span>
                    </td>
                    <td class="px-6 py-4 text-on-surface-variant text-base max-w-sm truncate">
                        {{ $service->short_description ?? '-' }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $service->status === 'published' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-surface-container-low text-on-surface-variant border border-surface-container-highest' }}">
                            {{ $service->status === 'published' ? 'Diterbitkan' : 'Draft' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.services.edit', $service) }}" class="p-2 text-outline hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                <x-lucide-edit class="w-4 h-4" />
                            </a>
                            <form action="{{ route('admin.services.destroy', $service) }}" method="POST" class="inline" data-no-alert>
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-outline hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete" onclick="return confirm('Apakah Anda yakin ingin menghapus layanan ini?')">
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-outline">
                        <div class="flex flex-col items-center justify-center">
                            <x-lucide-package class="w-12 h-12 text-outline-variant mx-auto mb-4" />
                            <h3 class="text-xl font-bold text-on-surface">Belum ada data layanan</h3>
                            <p class="text-outline mt-2 max-w-md mx-auto">Klik tombol "Tambah Layanan" di atas untuk membuat layanan pertama yang akan tampil di situs web Anda.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
