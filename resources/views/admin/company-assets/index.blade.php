<x-admin-layout>
    <div class="space-y-6" x-data="{ activeTab: 'global' }">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-surface-container overflow-hidden">
            <!-- Tabs Header -->
            <div class="flex overflow-x-auto border-b border-surface-container custom-scrollbar bg-surface-container-low/50">
                <button @click="activeTab = 'global'"
                        class="px-6 py-4 text-sm font-medium transition-colors border-b-2 whitespace-nowrap"
                        :class="activeTab === 'global' ? 'border-primary text-primary font-bold' : 'border-transparent text-outline hover:text-on-surface-variant hover:border-surface-container-highest'">
                    BKJ Group (All Pages)
                </button>
                @foreach($subsidiaries as $sub)
                    <button @click="activeTab = '{{ $sub->slug }}'"
                            class="px-6 py-4 text-sm font-medium transition-colors border-b-2 whitespace-nowrap"
                            :class="activeTab === '{{ $sub->slug }}' ? 'border-primary text-primary font-bold' : 'border-transparent text-outline hover:text-on-surface-variant hover:border-surface-container-highest'">
                        {{ $sub->name }}
                    </button>
                @endforeach
            </div>
                ...
                <div x-show="activeTab === 'global'" x-cloak class="space-y-6">
                    <form action="{{ route('admin.company-assets.update-global') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <!-- Global Icon Card -->
                            <div class="bg-white rounded-2xl border border-surface-container shadow-sm overflow-hidden flex flex-col">
                                <div class="p-6 border-b border-surface-container-low bg-surface-container-low/30">
                                    ...
                                    <p class="text-xs text-outline mt-1">This icon represents the brand logo inside public sections and pages of all general pages.</p>
                                </div>
                                <div class="p-6 flex-1 flex flex-col justify-between space-y-6">
                                    <!-- Preview -->
                                    <div class="flex flex-col items-center justify-center p-4 bg-surface-container-low border border-dashed border-surface-container-highest rounded-xl h-44">
                            ...
                            <div class="bg-white rounded-2xl border border-surface-container shadow-sm overflow-hidden flex flex-col">
                                <div class="p-6 border-b border-surface-container-low bg-surface-container-low/30">
                                    ...
                                    <p class="text-xs text-outline mt-1">This favicon is rendered dynamically in the HTML head when users view all general pages of the company profile.</p>
                                </div>
                                <div class="p-6 flex-1 flex flex-col justify-between space-y-6">
                                    <!-- Preview -->
                                    <div class="flex flex-col items-center justify-center p-4 bg-surface-container-low border border-dashed border-surface-container-highest rounded-xl h-44">
                                        @if(!empty($globalSettings['global_favicon']) && \Illuminate\Support\Facades\Storage::disk('public')->exists($globalSettings['global_favicon']))
                                            <div class="p-4 bg-white border border-surface-container rounded-xl shadow-sm flex items-center justify-center">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <div class="bg-white rounded-2xl border border-surface-container shadow-sm overflow-hidden flex flex-col">
                                    <div class="p-6 border-b border-surface-container-low bg-surface-container-low/30">
                                        ...
                                        <p class="text-xs text-outline mt-1">This icon represents the brand logo inside public sections and pages.</p>
                                    </div>
                                    <div class="p-6 flex-1 flex flex-col justify-between space-y-6">
                                        <!-- Preview -->
                                        <div class="flex flex-col items-center justify-center p-4 bg-surface-container-low border border-dashed border-surface-container-highest rounded-xl h-44">
                                ...
                                <div class="bg-white rounded-2xl border border-surface-container shadow-sm overflow-hidden flex flex-col">
                                    <div class="p-6 border-b border-surface-container-low bg-surface-container-low/30">
                                        ...
                                        <p class="text-xs text-outline mt-1">This favicon is rendered dynamically in the HTML head when users view this company's section.</p>
                                    </div>
                                    <div class="p-6 flex-1 flex flex-col justify-between space-y-6">
                                        <!-- Preview -->
                                        <div class="flex flex-col items-center justify-center p-4 bg-surface-container-low border border-dashed border-surface-container-highest rounded-xl h-44">
