    ...
    <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 max-w-4xl">
        <form action="{{ route('admin.subsidiaries.store') }}" method="POST" id="subsidiary-form" enctype="multipart/form-data">
            ...
            <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-outline-variant/20">
                <a href="{{ route('admin.subsidiaries.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-surface-container text-on-surface-variant border border-outline-variant rounded-lg hover:bg-surface-container-high transition-colors font-medium text-sm shadow-sm">Cancel</a>
                <x-primary-button>
